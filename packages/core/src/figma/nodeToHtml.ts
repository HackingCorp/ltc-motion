...
function textCss(node: FigmaNodeDocument, styles: string[]): void {
  ...
  if (typeof s.fontFamily === "string") styles.push(`font-family: '${s.fontFamily}'`);
...
function renderChildren(node: FigmaNodeDocument, ctx: RenderContext): string {
  const childHtml: string[] = [];
  for (const child of childDocuments(node)) {
    const rendered = renderNodeHtml(child, ctx, false);
    if (rendered.length > 0) childHtml.push(rendered);
  }
  return childHtml.length > 0 ? `\n${childHtml.join("\n")}\n` : "";
}

function renderNodeHtml(node: FigmaNodeDocument, ctx: RenderContext, isRoot: boolean): string {
  if (node.visible === false) return "";
  const slug = uniqueSlug(ctx, node.name);
  const style = [...geometryCss(node, ctx, isRoot), ...decorationCss(node, ctx)].join("; ");
  ...
  return `<div ${idAttrs} style="${style}">${renderChildren(node, ctx)}</div>`;
}
