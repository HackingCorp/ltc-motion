...
> **Fork note (ltc-motion).** You are in [HackingCorp/ltc-motion](https://github.com/HackingCorp/ltc-motion), a synced fork of heygen-com/hyperframes. Install skills **from this repo** so fork additions are included: `npx skills add HackingCorp/ltc-motion --all -y`. Audio without any credential: `npx hyperframes tts "..." --provider edgetts --voice fr-FR-DeniseNeural -o voice.wav` (free neural voices, `pip install edge-tts`) and `npx hyperframes music "<mood>" -o bgm.wav` (original music — Lyria with GEMINI_API_KEY, else local MusicGen); voice cloning via `--provider fishspeech` (FISH_SPEECH_RUNPOD_ENDPOINT + RUNPOD_API_KEY). AI images/videos: the self-hosted `hyperframes image|video` RunPod endpoints are dormant — generate those assets on a managed platform (e.g. Higgsfield, driveable through its MCP) and drop the files into the composition as regular assets. Making a video ad? Follow the `/ad-spot` skill: AI footage for humans/ambiance only, HTML panels for anything textual on screen, segmented voice-over — it encodes the review failures we already paid for. Other fork features (browser export, French Studio locale, GCP parity tooling, 9:16 blocks) are indexed in [`ROADMAP-LTC.md`](ROADMAP-LTC.md) and summarized at the top of [`README.md`](README.md). Everything else in this file applies unchanged.
